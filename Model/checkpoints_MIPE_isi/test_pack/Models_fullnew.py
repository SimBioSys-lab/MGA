...
def _find_chain_spans_1d(tokens: torch.Tensor, eoc_id: int = 24, pad_id: int = 1):
    ...
    Old behavior usually used EOC positions directly and could include EOC tokens
    inside a chain or miss later antigen chains.

    This fixed version:
      1. reads the query row only
      2. stops at padding
      3. splits at every EOC
      4. excludes EOC tokens from the attended chain spans
...
class CoreModel(nn.Module):
    ...
    Keeps old outer residual/norm behavior:
      res = x
      x_attn = attention(...)
      x = norm_layers[i](res + x_attn)

      res = x
      x_ffn = ffn(...)
      x = ffn_norm_layers[i](res + x_ffn)
    """
    def __init__(
        self,
        vocab_size: int,
        ...
        # Keep DyM/DropPath hooks, initialized identity if your DyM has zero init.
